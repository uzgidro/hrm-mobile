...
describe('parallel pagination concurrency cap', () => {
  it('never has more than 4 page requests in flight (5000 employees → 10 pages)', async () => {
    let active = 0;
    let maxActive = 0;

    mock.onGet(EMPLOYEES_LIST).reply(async (config) => {
      const page = Number(config.params?.page ?? 1);
      if (page === 1) return [200, { items: items(500), total: 5000 }];
      active += 1;
      maxActive = Math.max(maxActive, active);
      await new Promise((r) => setTimeout(r, 5));
      active -= 1;
      return [200, { items: items(500), total: 5000 }];
    });

    const result = await fetchAllEmployees();
    // 10 pages of 500 → 5000 merged items, request count = 10.
