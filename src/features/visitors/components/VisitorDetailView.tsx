...
// The body of the visitor detail — extracted so it can render either as the
// pushed route's content (phone, `embedded` falsy) or embedded inside the
// tablet split-view's right pane (`embedded` truthy). Embedded mode drops the
// safe-area root (the split pane already sits inside the list screen's own
...
export function VisitorDetailView({ id, embedded = false }: { id: number; embedded?: boolean }) {
  ...
  // same top safe-area root as before. A plain function (not a component
  // defined during render) so children keep their identity/state across
  // re-renders instead of being torn down and remounted.
...
const makeStyles = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    header: {
      flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10, gap: 4,
    },
