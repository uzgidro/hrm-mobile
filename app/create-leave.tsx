...
const LEAVE_TYPES = ["Xizmat topshirig'i", 'Kasallik', "Ta'til", 'Shaxsiy sabab', 'Boshqa'];

function DateTimePicker({ value, visible, title, minDate, onConfirm, onClose }: {
...
function TypeSheet({ visible, selected, onSelect, onClose }: {
...
export default function CreateLeaveScreen() {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <TypeSheet visible={showTypeSheet} selected={leaveType} onSelect={setLeaveType} onClose={() => setShowTypeSheet(false)} />
      <DateTimePicker visible={activePicker === 'start'} title="Boshlanish vaqti" value={startDate}
        onConfirm={(v) => { setStartDate(v); if (v.isAfter(endDate)) setEndDate(v.add(1, 'hour')); }} onClose={() => setActivePicker(null)} />
      <DateTimePicker visible={activePicker === 'end'} title="Tugash vaqti" value={endDate} minDate={startDate} onConfirm={setEndDate} onClose={() => setActivePicker(null)} />
    </SafeAreaView>
  );
}
...
const makeDp = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    navText: { color: c.text, fontSize: 16, fontWeight: '700' },
    ...
    timeBtnText: { fontSize: 16, color: c.primaryLight, fontWeight: '700' },
...
const makeTs = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    check: { fontSize: 16, color: c.primaryLight, fontWeight: '700' },
