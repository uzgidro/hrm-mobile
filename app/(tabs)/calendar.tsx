...
import { TURNSTILE_ATTENDANCE_EVENTS } from '../../src/api/urls';
import { COLORS } from '../../src/constants';
import { AttendanceEvent } from '../../src/types';
...
interface DayStat {
  ...
  isAbsent: boolean; // working day but no attendance
}

function DonutChart({ attended, total, size = 160 }: { attended: number; total: number; size?: number }) {
  const radius = 58;
  ...
  // 12 o'clockdan boshlash uchun -90° rotate (rotation prop emas, transform string)
  ...
  return (
    <Svg width={size} height={size}>
      ...
      {/* Missed arc — attended dan keyin boshlanadi */}
      {missedDash > 0 && (
        <Circle
          cx={cx} cy={cy} r={radius} fill="none"
      ...
      {/* Center fill */}
      <Circle cx={cx} cy={cy} r={radius - strokeWidth - 4} fill={COLORS.card} />
    </Svg>
  );
}
...
export default function CalendarScreen() {
  ...
  const { data: events = [], refetch } = useQuery<AttendanceEvent[]>({
    queryKey: ['attendance', employee?.id, monthKey],
    queryFn: () =>
      apiClient.get(TURNSTILE_ATTENDANCE_EVENTS, {
        params: {
          date_from: currentMonth.format('YYYY-MM-DD'),
          date_to: currentMonth.endOf('month').format('YYYY-MM-DD'),
  ...
  const firstDayOfWeek = (currentMonth.day() + 6) % 7; // Mon=0
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const hasAttendance = dayEvents.length > 0;
    const isFuture = dayjs(dateStr).isAfter(dayjs(), 'day');
    const isAbsent = !isWeekend && !hasAttendance && !isFuture;

    calendarDays.push({ date: dateStr, isWeekend, isToday, hasAttendance, isAbsent });
  }

  // Selected day events — birinchi kirish va oxirgi chiqish
  const selectedEvents = eventsByDate[selectedDate] || [];
  const entryEvents = selectedEvents
    .filter((e) => e.direction_type === 'entrance' || e.check_in_out_type === 1)
    .sort((a, b) => dayjs(a.happen_time).diff(dayjs(b.happen_time)));
  const exitEvents = selectedEvents
    .filter((e) => e.direction_type === 'exit' || e.check_in_out_type === 2)
    .sort((a, b) => dayjs(a.happen_time).diff(dayjs(b.happen_time)));
  const selEntry = entryEvents[0];       // eng erta kirish
  const selExit = exitEvents[exitEvents.length - 1]; // eng kech chiqish
  ...
  const attendedDays = Object.keys(eventsByDate).filter((dt) => {
    const dw = (dayjs(dt).day() + 6) % 7;
    return dw < 5;
  }).length;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      >
        <Text style={styles.pageTitle}>Taqvim</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.monthNav}>
            ...
            <Text style={styles.monthTitle}>
              {MONTHS_UZ[currentMonth.month()]} {currentMonth.year()}
            </Text>
            <TouchableOpacity
              style={styles.navBtn}
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    day.isAbsent && styles.dayCellAbsent,
                    day.hasAttendance && !day.isWeekend && styles.dayCellPresent,
                ...
                >
                  <Text style={[
                    styles.dayText,
                    day.isWeekend && styles.dayTextWeekend,
                    day.isAbsent && styles.dayTextAbsent,
        ...
        {/* Selected day entry/exit */}
        <View style={styles.card}>
          <View style={styles.entryExitRow}>
            <View style={styles.entryExitItem}>
              <Text style={styles.entryExitTime}>
                {selEntry ? dayjs(selEntry.happen_time).format('HH:mm') : '--:--'}
              </Text>
            ...
            <View style={styles.entryExitItem}>
              <Text style={styles.entryExitTime}>
                {selExit ? dayjs(selExit.happen_time).format('HH:mm') : '--:--'}
              </Text>
        ...
        {employee?.working_hours_start && (
          <View style={styles.card}>
            <View style={styles.cardTitleRow}>
              ...
              <Text style={styles.cardTitle}>Ish jadvali</Text>
            </View>
        ...
        <View style={styles.card}>
          <View style={styles.cardTitleRow}>
            ...
            <Text style={styles.cardTitle}>
              Oylik statistika {MONTHS_UZ[currentMonth.month()]} {currentMonth.year()}
            </Text>
          </View>

          <View style={styles.statsRow}>
            <View style={styles.donutWrapper}>
              <DonutChart attended={attendedDays} total={workDays} />
            ...
            <View style={styles.statsLegend}>
              <View style={styles.legendItem}>
                ...
                <Text style={styles.legendText}>
                  {missedDays} ({workDays > 0 ? Math.round((missedDays / workDays) * 100) : 0}%)
                </Text>
              ...
              <View style={styles.legendItem}>
                ...
                <Text style={styles.legendText}>
                  {remainingWorkDays} ({workDays > 0 ? Math.round((remainingWorkDays / workDays) * 100) : 0}%)
                </Text>
          ...
          <Text style={styles.hoursText}>{totalPlannedHours} soat</Text>
...
const styles = StyleSheet.create({
  ...
  navBtn: { width: 40, height: 40, backgroundColor: COLORS.cardBorder, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  navBtnText: { color: COLORS.text, fontSize: 16, fontWeight: '700' },
  monthTitle: { fontSize: 16, fontWeight: '700', color: COLORS.text },

  weekRow: { flexDirection: 'row', marginBottom: 8 },
  weekDayLabel: { flex: 1, textAlign: 'center', fontSize: 12, color: COLORS.textMuted, fontWeight: '600' },
  ...
  dayCellWeekend: { backgroundColor: COLORS.weekend + '33', borderRadius: 10 },
  dayCellAbsent: { backgroundColor: COLORS.absent + '66', borderRadius: 10 },
  dayCellPresent: { backgroundColor: 'transparent' },
  dayCellToday: { backgroundColor: COLORS.primary, borderRadius: 10 },
  dayCellSelected: { borderWidth: 2, borderColor: COLORS.primaryLight, borderRadius: 10 },

  dayText: { fontSize: 14, fontWeight: '600', color: COLORS.text },
  dayTextWeekend: { color: COLORS.primaryLight },
  dayTextAbsent: { color: '#F87171' },
  ...
  entryExitTime: { fontSize: 22, fontWeight: '700', color: COLORS.text, letterSpacing: 1 },
  entryExitLabel: { fontSize: 12, color: COLORS.textMuted, fontWeight: '500' },
  entryExitDivider: { width: 1, height: 36, backgroundColor: COLORS.cardBorder, marginHorizontal: 16 },

  scheduleRow: { flexDirection: 'row', gap: 20 },
  scheduleItem: { flex: 1 },
  scheduleValue: { fontSize: 16, fontWeight: '700', color: COLORS.text },
  scheduleLabel: { fontSize: 12, color: COLORS.textMuted, marginTop: 4 },
  ...
  eventDir: { fontSize: 13, color: COLORS.textSecondary },
  ...
  donutCenter: {
    position: 'absolute', fontSize: 28, fontWeight: '800', color: COLORS.text,
  },
  statsLegend: { flex: 1, gap: 16 },
  legendItem: { gap: 4 },
  legendDot: { width: 10, height: 10, borderRadius: 5 },
  legendText: { fontSize: 18, fontWeight: '700', color: COLORS.text },
  legendLabel: { fontSize: 12, color: COLORS.textSecondary },
  hoursText: { fontSize: 22, fontWeight: '700', color: COLORS.text, marginTop: 12 },
  hoursLabel: { fontSize: 12, color: COLORS.textMuted, marginTop: 2 },
});
