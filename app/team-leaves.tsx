...
export default function TeamLeavesScreen() {
  ...
  const filtered = useMemo(() => {
    return allLeaves
      .filter((l) => {
        // Branch filter: only show leaves for employees in this org branch
        if (orgBranchId && l.employee?.department?.organization_branch_id !== orgBranchId) {
          return false;
        }
...
const styles = StyleSheet.create({
  ...
  monthFilterWrapper: { flexShrink: 0, borderBottomWidth: 1, borderBottomColor: COLORS.cardBorder },
  monthRow: {
  ...
  monthChipText: { fontSize: 13, color: COLORS.textMuted, fontWeight: '600' },
