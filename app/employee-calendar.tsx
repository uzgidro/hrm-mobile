...
function DonutChart({ attended, total, size = 160 }: { attended: number; total: number; size?: number }) {
  ...
  return (
    <Svg width={size} height={size}>
      <Circle cx={cx} cy={cy} r={radius} fill="none" stroke={COLORS.cardBorder} strokeWidth={strokeWidth} />
      {attendedDash > 0 && (
        <Circle
          cx={cx} cy={cy} r={radius} fill="none"
          stroke={COLORS.textSecondary} strokeWidth={strokeWidth}
          strokeDasharray={`${attendedDash} ${circumference - attendedDash}`}
          strokeDashoffset={0} strokeLinecap="round" transform={rotate}
        />
      )}
      {missedDash > 0 && (
        <Circle
          cx={cx} cy={cy} r={radius} fill="none"
          stroke={COLORS.absent} strokeWidth={strokeWidth}
          strokeDasharray={`${missedDash} ${circumference - missedDash}`}
          strokeDashoffset={-attendedDash} strokeLinecap="round" transform={rotate}
        />
      )}
...
export default function EmployeeCalendarScreen() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    calendarDays.push({
      ...
      isWeekend,
      isToday,
      hasAttendance: dayEvents.length > 0,
      isAbsent: !isWeekend && dayEvents.length === 0 && !isFuture,
    });
  }
...
const styles = StyleSheet.create({
  ...
  navBtn: {
    width: 40, height: 40, backgroundColor: COLORS.cardBorder,
  ...
  donutCenter: { position: 'absolute', fontSize: 28, fontWeight: '800', color: COLORS.text },
