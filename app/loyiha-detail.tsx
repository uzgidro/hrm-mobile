...
import {
  WORKSPACE_DETAIL, COLUMNS_LIST, CARDS_LIST, CARD_COMPLETE, CARD_UNCOMPLETE,
} from '../src/api/urls';
import { useTheme, useThemedStyles } from '../src/theme/ThemeProvider';
import type { ThemeColors } from '../src/theme/palettes';
import { ScreenHeader, HeaderAction } from '../src/components/ScreenHeader';
import { Icon } from '../src/components/Icon';
import { isMasterAdmin } from '../src/utils/roles';
  ...
  const submitModal = async () => {
    ...
    try {
      if (modal.type === 'column') {
        await apiClient.post(COLUMNS_LIST, { name: field1.trim(), workspace_id: workspaceId });
        qc.invalidateQueries({ queryKey: ['workspace', workspaceId] });
      } else {
        await apiClient.post(CARDS_LIST, {
          title: field1.trim(),
          description: field2.trim() || undefined,
          column_id: modal.columnId,
        });
        qc.invalidateQueries({ queryKey: ['cards', modal.columnId] });
      }
      setModal(null);
    } catch (e: any) {
      Alert.alert('Xatolik', e?.response?.data?.detail || 'Saqlashda xatolik');
    } finally {
      setBusy(false);
    }
  };

  const toggleComplete = async (card: WorkspaceCard) => {
    try {
      await apiClient.post(card.is_completed ? CARD_UNCOMPLETE(card.id) : CARD_COMPLETE(card.id));
      qc.invalidateQueries({ queryKey: ['cards', card.column_id] });
    } catch (e: any) {
  ...
  const onDelete = () => {
    Alert.alert('O\'chirish', "Loyihani o'chirishni xohlaysizmi?", [
      { text: 'Bekor', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await apiClient.delete(WORKSPACE_DETAIL(workspaceId));
            qc.invalidateQueries({ queryKey: ['workspaces'] });
            router.back();
          } catch (e: any) {
            Alert.alert('Xatolik', e?.response?.data?.detail || "O'chirishda xatolik (faqat yaratuvchi o'chira oladi)");
          }
        },
      },
    ]);
                  ...
                  {loading ? (
                    ...
                    cards.map((cd) => (
                      <View key={cd.id} style={[styles.taskCard, cd.is_completed && styles.taskDone]}>
                        <TouchableOpacity onPress={() => toggleComplete(cd)} hitSlop={8} style={[styles.checkbox, cd.is_completed && styles.checkboxOn]}>
