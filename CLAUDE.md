...
**Release / CI-CD.** `master` is the single long-lived branch; work happens on short-lived `feat/*` / `fix/*` / `refactor/*` branches merged into it via PR. (A second long-lived `refactoring` dev branch existed until 2026-08-13 and was deleted once its work landed in `master` — CI no longer references it.) CI (`.github/workflows/`): `ci.yml` (typecheck/lint/jest on PR + push to `master`), `ota.yml` (push to `master` → `eas update --channel production`, an over-the-air JS/asset update — **every merge to `master` ships to users**), `release.yml` (tag `v<app.json version>` on `master` → `eas build --profile production --auto-submit` to the Google Play **internal** track; promotion to production stays manual in Play Console). `eas.json` uses `appVersionSource: remote` (EAS owns `versionCode`); `runtimeVersion` policy is `appVersion`.
...
**State**: **Zustand**, not Redux/Context, for app state. `authStore` (user, auth flags, login/logout) and `prefsStore` (theme preference, hydrated on launch). React Query owns all server state. Role predicates (`isMasterAdmin`/`isEmployee`/`isHR`, …) live **only** in `src/utils/roles.ts` — `authStore` once carried duplicates that compared `multi_org_employee_role` directly and silently denied access when it arrived as an array; resolve roles through `roles.ts` (which uses `getMultiOrgRoles()`), never off the store.
